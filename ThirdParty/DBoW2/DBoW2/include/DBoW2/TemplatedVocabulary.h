...
public:
  ...
  void load_ifstream(const std::string &filename);
  ...
  virtual void load(std::ifstream& filestream);
  /**
...
void TemplatedVocabulary<TDescriptor,F>::load(const std::string &filename)
{
  ...
  std::cout << "end load file" << std::endl;
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::load_ifstream(const std::string &filename)
{
  std::cout << "loading file" << std::endl;  
  ...
  {
    std::cout << "Could not open vocabulary file " << filename << std::endl;
    return;
  }
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::clear()
{
  m_nodes.clear();
  m_words.clear();
}
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::load(std::ifstream& filestream)
{  
  ...
  while (std::getline(filestream,line))
  {
    ...
    int pos_leftbracket = line.find('{');
    
    if (pos_leftbracket != std::string::npos)
    {      
      if (mode == 1)
      {
        line = line.substr(pos_leftbracket+1);
        NodeId nid = 0;
        NodeId pid = 0;
        WordValue weight = 0.f;
                
        
        //char* line_ch;
        //line_ch = new char[line.size()+1]; 
        //memcpy(line_ch,line.c_str(),line.size()+1);
        
        char line_ch[1024];
        std::size_t length = line.copy(line_ch, line.size());
        line_ch[length] = '\0';
        ...
        while (pch!= NULL)
        {
          std::string entry_token(pch);
          //std::cout << entry_token << std::endl;
          int pos_twopoints = entry_token.find(':');
    
          if (pos_twopoints != std::string::npos)
          {
            entry_name = trim(entry_token.substr(0,pos_twopoints));
            entry_value = trim(entry_token.substr(pos_twopoints+1));
    
            if (!entry_value.empty())
            {
              if(strcmp(entry_name.c_str(),"nodeId")==0)
                nid = atoi(entry_value.c_str());
              if(strcmp(entry_name.c_str(),"parentId")==0)
                pid = atoi(entry_value.c_str());
              if(strcmp(entry_name.c_str(),"weight")==0)
                weight = (WordValue) (atof(entry_value.c_str()));
            }
          }
        ...
        int pos_leftquote = line.find_first_of('"');
        int pos_rightquote = line.find_last_of('"');
        int length_d = pos_rightquote - pos_leftquote;
        
        std::string d = line.substr(pos_leftquote+1, length_d);
      ...
      {        
        ...
        while (pch!= NULL)
        {
          ...
          if (pos_twopoints != std::string::npos)
          {
            ...
            if (!entry_value.empty())
            {
              if(strcmp(entry_name.c_str(),"wordId")==0)
                wid = atoi(entry_value.c_str());
              if(strcmp(entry_name.c_str(),"nodeId")==0)
                nid = atoi(entry_value.c_str());
            }
          }
    ...
    int pos_twopoints = line.find(':');
    
    if (pos_twopoints != std::string::npos)
    {
      entry_name = trim(line.substr(0,pos_twopoints));
      entry_value = trim(line.substr(pos_twopoints+1));
      
      if (!entry_value.empty())
      {
        if(strcmp(entry_name.c_str(),"k")==0)
          m_k = atoi(entry_value.c_str());
        if(strcmp(entry_name.c_str(),"L")==0)
          m_L = atoi(entry_value.c_str());
        if(strcmp(entry_name.c_str(),"scoringType")==0)
          m_scoring = (ScoringType) (atoi(entry_value.c_str()));
        if(strcmp(entry_name.c_str(),"weightingType")==0)
          m_weighting = (WeightingType) (atoi(entry_value.c_str()));
      }
      else //it is a change of mode
      {
        //std::cout << entry_name << std::endl;
        if(strcmp(entry_name.c_str(),"nodes")==0)
         mode = 1;
        if(strcmp(entry_name.c_str(),"words")==0)
        {
         mode = 2;
         m_nodes.resize(num_nodes);
         m_words.resize(num_nodes-1);
        }
      }
    }
  }
  ...
  //std::cout << m_k << std::endl
            //<< m_L << std::endl
            //<< m_weighting << std::endl
            //<< m_scoring << std::endl;
            
  //for (int i=0; i< 100; i++)
  //{
    //std::cout << "Node " << i << std::endl;
    //std::cout << "  node id: " << m_nodes[i].id << std::endl;
    //std::cout << "  parent id: " << m_nodes[i].parent << std::endl;
    //std::cout << "  weight id: " << m_nodes[i].weight << std::endl;
    //std::cout << "  word id: " << m_nodes[i].word_id << std::endl;
    //std::cout << "  descriptor: " << F::toString(m_nodes[i].descriptor) << std::endl;    
  //}
  
  std::cout << "Showing loaded info for random node to check that vocabulary file load is OK" << std::endl;
  std::cout << "Node " << 1082046 << std::endl;
  std::cout << "  node id: " << m_nodes[1082046].id << std::endl;
  std::cout << "  parent id: " << m_nodes[1082046].parent << std::endl;
  std::cout << "  weight id: " << m_nodes[1082046].weight << std::endl;
  std::cout << "  word id: " << m_nodes[1082046].word_id << std::endl;
  std::cout << "  descriptor: " << F::toString(m_nodes[1082046].descriptor) << std::endl;  
